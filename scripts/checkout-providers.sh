...
curl 'https://api.github.com/orgs/terraform-providers/repos?per_page=100' > repos.json
curl 'https://api.github.com/orgs/terraform-providers/repos?per_page=100&page=2' > repos2.json
...
for git_uri in $(cat repos2.json | jq -r '.[] | select( .archived == false ) | .ssh_url'); do
    repo_dir=$(basename $git_uri .git)
    skip=$(skip_provider "$repo_dir")
    if [[ ! -d $repo_dir ]] && [[ -z "$skip" ]]
    then
